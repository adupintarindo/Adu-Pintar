...
import { registerSchoolAccount, registerStudentAccount, registerTeacherAccount } from "@/lib/school-management"
import { z } from "zod"
...
const studentRegisterSchema = z.object({
  ...
  password: z.string().min(8).max(128),
  gender: z.enum(["M", "F"]),
  dateOfBirth: z.string().min(6).max(16),
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (role === "student") {
      const parsed = parseAndValidateBody(body, studentRegisterSchema)
      ...
      const result = await registerStudentAccount({
        ...
        schoolProvince: parsed.data.schoolProvince,
      ...
      response.cookies.set(
        "user_session",
        encodeSessionCookie(attachSessionId(result.authUser)),
        buildAuthCookieOptions(60 * 60 * 24),
      )
      logApiRequest(request, 200, { role: "student", mode: result.mode })
      return response
    }

    /* ---- Teacher registration ---- */
    if (role === "teacher") {
      const parsed = parseAndValidateBody(body, teacherRegisterSchema)
      if (!parsed.data || parsed.errorResponse) {
        logApiRequest(request, 400, { reason: "validation" })
        return parsed.errorResponse!
      }
      ...
      logApiRequest(request, 200, { role: "teacher", mode: result.mode })
      return response
    }

    /* ---- School registration ---- */
    if (role !== "school") {
      logApiRequest(request, 400, { reason: "unsupported_role", role })
      return NextResponse.json({ error: "Peran tidak dikenali." }, { status: 400 })
    }
